...
class TestPotpourri(unittest.TestCase):
    ...
    def test_all(self):
        ...
        aln= aligner('CPELIRKNCANTH', 'PREKRLICAN', method='global', gap_open=-0.5)
        assert list(aln.seq1) == ['C', 'P', '-', 'E', '-', '-', 'L', 'I', 'R', 'K', 'N', 'C', 'A', 'N', 'T', 'H']
        assert list(aln.seq2) == ['-', 'P', 'R', 'E', 'K', 'R', 'L', 'I', '-', '-', '-', 'C', 'A', 'N', '-', '-']
        ...
        assert list(aln.seq2) == ['T', 'A', 'A', 'T' ]
        assert list(aln.seq1) == ['T', 'A', 'A', 'T' ]
        aln = aligner('AAATAATAAA', 'TATAT', method='glocal', gap_open=-1, matrix=DNAFULL)
        assert list(aln.seq1) == ['T', 'A', '-', 'A', 'T' ]
        assert list(aln.seq2) == ['T', 'A', 'T', 'A', 'T' ]

        aln = aligner('TATATAAA', 'CCTATAT', method='glocal', gap_open=-1, matrix=DNAFULL)
        assert (aln.seq1, aln.seq2) == ('--TATAT', 'CCTATAT' ), aln

        aln = aligner('CCTATAT', 'TATATAAA',method='glocal', gap_open=-1, matrix=DNAFULL)
        assert list(aln.seq2) == ['-', '-', 'T', 'A', 'T', 'A', 'T' ]
        ...
        assert list(aln.seq1) == ['P','A','A']
        assert list(aln.seq2) == ['P','A','-']
        aln = aligner('TAATTC', 'TAAT', method='global', matrix=DNAFULL, gap_open=-10, gap_extend=-1)
